...
## 2026-08-18: Fantasy and Superhero decks authored — 226 briefs each, zero images generated

**Context.** Second half of the same day as the Nano Banana Pro pass (entry
above, not restated). The owner directed: "create the fantasy version of the
decks, which is going to be world of Warcraft cards style and hero's version
a comic book style. Go and define each card, update the editor, and any
other things that need to be updated" — and explicitly "Don't render the
images." Commits `94ac22a`, `3a4a62e`, `a9fe6f3`. Both packs went from
`draft:true` scaffolds to fully authored worlds: 226 briefs each, bios on
the 98 non-grade entries per pack, draft flags cleared, docs generated, and
all 452 new entries stay `status: "planned"` — zero images generated.

**Decisions.**
- The style pivot is the owner's, not derived: Fantasy = World of Warcraft
  card style (the scaffold's painterly/comic hybrid rewritten to fully
  hand-painted Warcraft-TCG language); Superhero = comic book style.
- Draft flags cleared as part of "define each card" — authored-but-
  ungenerated is the same lifecycle state sci-fi shipped in, and clearing
  `draft` turns on the coverage check and doc generation for both packs.
  "Complete world minus images" is now a validator-enforced state, not a
  claim.
- Bios only on the 98 non-grade entries per pack, matching sci-fi's exact
  split; grades carry none.
- One object metaphor per lineage, held across all four grades, in every
  world — sci-fi's survey drone becomes fantasy's scrying orb and
  superhero's rooftop searchlight — so a morph chain reads as one card
  growing, consistently, across worlds.
- The comic idiom's known frame-priming risk (found the hard way in the
  morning's sci-fi run, not restated here) was answered inside the style
  paragraph itself rather than deferred to generation time: Superhero
  appends its own exclusions AFTER the validator's no-text rule substring —
  no speech bubbles, no caption boxes, no sound-effect lettering, no panel
  gutters — and frames every shot as "a full-bleed splash-page
  illustration." Both new style paragraphs carry the "describe what the
  frame IS" lesson from day one, rather than re-deriving it at generation
  time.
- "Don't render the images" honored absolutely: zero generation calls this
  session.

**Learnings.**
- A cross-world uniqueness sweep is worth running after any multi-world
  authoring pass, not just a per-world one: 21 superhero prompts had been
  echoed verbatim from their sci-fi anchors — the sci-fi scenes were already
  tech-flavoured, so the echo was frictionless to write and easy to miss by
  eye. All 21 re-dressed; all 678 prompts across three worlds confirmed
  unique. The check is three lines of node, cheap enough to run every time.
- The security gate caught a Medium in this same session's own uncommitted
  fix — the strongest argument yet for gating uncommitted work, not only
  landed commits: moving image READS to the manifest's own theme
  (`loadedTheme`) while writes still posted the ambient `selected` tab
  theme meant a click landing in the tab-switch window could write one
  pack's bio or bytes into a DIFFERENT pack's file. Fixed by defaulting the
  save call's POST theme to `manifest?.theme` (the scaffold-pack path
  overrides explicitly) and adding a monotonic sequence guard to the
  refresh path so a stale response can't pin the manifest. Verified
  through the studio's own Save button: only `fantasy.json` changed.
- The tab-switch 404 race that motivated `loadedTheme` in the first place:
  for one render after switching pack tabs, the old pack's rows still
  request the new theme's variant URLs — 4 observed 404s per switch, live.
  General shape, reads and writes both: a server-echoed identifier that
  travels WITH the data it describes beats separately-tracked selection
  state.
- Injection-proving a validator beats eyeballing it, with a nuance learned
  the hard way: the new backtick guard (pack free text lands inside
  generated-doc code fences) was proven by injecting a backtick and
  watching `data:validate` name the exact entry — but the first attempt was
  masked by the doc-staleness check firing first. When proving a guard,
  inject into a field ONLY that guard can catch (`alt`, which the generated
  docs never print), or an earlier-firing check will swallow the proof.
- `data:validate` reports doc staleness before content errors — worth
  knowing before trying to prove a content-level guard through the CLI.

**Preferences / proposed amendments (need owner consent).** Both flags
below were raised by this session's gates.
- STANDARDS §2.2 names only the `PROMPT_ROLES.md` doc pipeline; the
  `CRAFT_ART_*` family is generated by a sibling script and drift-checked
  by `data:validate` the same way — broaden §2.2 to name both families.
  (integration gate) (proposed amendment, needs the owner's consent)
- A client action that reads pack-scoped data via one server-echoed
  identifier must reuse that SAME identifier for any write it triggers,
  never a separately-tracked current-selection value — would have caught
  the `loadedTheme`/`selected` Medium above by construction. (security
  gate) (proposed amendment, needs the owner's consent)
